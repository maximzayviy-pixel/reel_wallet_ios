...
/** Ensures a verified Telegram Mini App user; throws Response on failure. */
export async function requireUser(req: NextRequest): Promise<{ tgId: number; user: TMAUser }> {
  ...
  if (!('ok' in ctx) || !ctx.ok) {
    throwJson(ctx.status, ctx.error);
  }
  return { tgId: ctx.tgId, user: ctx.user };
}

/** Ensures admin access. Admins are from ADMIN_TG_IDS or users.role = 'admin' in DB. */
