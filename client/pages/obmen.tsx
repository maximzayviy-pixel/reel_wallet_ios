...
export default function Obmen() {
  ...
  // загрузка tgId и баланса
  useEffect(() => {
    (async () => {
      try {
        const initData = (window as any)?.Telegram?.WebApp?.initData || "";
        const match = /"id":(\d+)/.exec(initData);
        if (match) {
        ...
        {!betaHidden && (
          <div className="mb-5 rounded-2xl border border-yellow-200 bg-yellow-50 text-yellow-900 p-4">
            ...
            <div className="text-sm opacity-90">
              Функции ещё дорабатываются — возможны баги и задержки загрузки
              превью.
            </div>
            <button
              onClick={() => {
