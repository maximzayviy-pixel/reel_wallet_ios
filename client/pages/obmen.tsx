...
type Gift = {
  ...
  tme_link: string; price_rub: number | null; value_rub: number | null;
  image_url: string | null; tgs_url: string | null; anim_url: string | null;
  model: string | null; backdrop: string | null; pattern: string | null;
  amount_issued: number | null; amount_total: number | null;
};
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
            ...
            return (
              ...
              >
                ...
                <div className="mt-2">
                  ...
                  <div className="text-sm font-semibold mt-1 text-emerald-600">{fmt(price)}</div>
                </div>
              </button>
            );
          })}
        ...
        {selected && (
          <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center z-50" onClick={()=>setSelected(null)}>
            <div className="bg-white rounded-3xl w-full sm:w-[520px] p-4 sm:p-6 m-2" onClick={e=>e.stopPropagation()}>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-[12px]">
                    {selected.model && (
                      <div className="rounded-xl bg-slate-50 p-2">
                        <div className="text-slate-500">Модель</div><div className="font-medium">{selected.model}</div>
                      </div>
                    )}
                    {selected.backdrop && (
                      <div className="rounded-xl bg-slate-50 p-2">
                        <div className="text-slate-500">Фон</div><div className="font-medium">{selected.backdrop}</div>
                      </div>
                    )}
                    {selected.pattern && (
                      <div className="rounded-xl bg-slate-50 p-2">
                        <div className="text-slate-500">Узор</div><div className="font-medium">{selected.pattern}</div>
                      </div>
                    )}
