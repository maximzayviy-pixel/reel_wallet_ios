// pages/obmen.tsx — Маркетплейс подарков (вкладка «Обмен»)
...
type Gift = {
  ...
  anim_url: string | null;     // сейчас у многих null — используем tgs
  tgs_url: string | null;
  model: string | null;
...
export default function Obmen() {
  ...
  const [betaSeen, setBetaSeen] = useState(false);

  // показать «Бета-версия магазина» один раз
  useEffect(() => {
    const k = "beta_banner_seen_v1";
    setBetaSeen(!!sessionStorage.getItem(k));
  ...
  const closeBeta = () => {
    sessionStorage.setItem("beta_banner_seen_v1", "1");
    setBetaSeen(true);
  };
    ...
    try {
      const r = await fetch("/api/gifts/buy", {
        ...
        headers: {
          ...
          // важный заголовок с init data для проверки на бэке
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        {!betaSeen && (
          <div className="mb-5 rounded-2xl border border-yellow-200 bg-yellow-50 text-yellow-900 p-4">
            ...
            <button
              onClick={closeBeta}
              className="mt-3 inline-flex items-center rounded-xl bg-yellow-500 text-white px-4 py-2 hover:bg-yellow-600 transition"
            >
              Окей
            </button>
          </div>
        ...
        {error && (
          <div className="text-red-600 border border-red-200 bg-red-50 rounded-xl p-3">
            {error}
          </div>
        )}
          ...
          {items.map((g) => {
            ...
            return (
              <button
                ...
                className="group relative rounded-3xl bg-white ring-1 ring-slate-200 hover:ring-slate-300 transition p-3 text-left shadow-sm hover:shadow-md"
              >
                <div className="relative aspect-square rounded-2xl bg-slate-50 overflow-hidden">
                  {/* анимация .tgs с фолбэком на постер */}
                  <StickerPlayer
        ...
        {selected && (
          ...
          >
            <div
              className="bg-white rounded-3xl w-full sm:w-[520px] p-4 sm:p-6 m-2 relative"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex gap-4 items-start">
                <div className="relative w-24 h-24 rounded-2xl bg-slate-50 overflow-hidden shrink-0">
                  <StickerPlayer
                ...
                <div className="flex-1">
                  ...
                  <div className="text-2xl font-bold mt-1">
                    {formatRub(selected.value_rub ?? selected.price_rub ?? 0)}
                  </div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                <button
                  className="h-11 rounded-xl ring-1 ring-slate-200 hover:bg-slate-50 transition"
                  onClick={() => setSelected(null)}
                >
                  Отмена
                </button>
                <button
                  className="h-11 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition disabled:opacity-60"
                  disabled={buying}
