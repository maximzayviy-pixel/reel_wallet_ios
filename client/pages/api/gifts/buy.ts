...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const ok = validateTelegramInitData(initData, BOT_TOKEN);
  ...
  if (gErr || !gift || gift.enabled === false) return res.status(400).json({ ok: false, error: "gift_not_available" });

  // найти user_id по tg_id
  const { data: userRec, error: uErr } = await supabase.from("users").select("id").eq("tg_id", tgUser.id).maybeSingle();
  if (uErr || !userRec) return res.status(400).json({ ok: false, error: "user_not_found" });
