...
export default function ScanKozen() {
  ...
  useEffect(() => {
    ...
    // @ts-ignore: also inverted may be unsupported in older builds
        ...
        const c = await reader.decodeFromVideoDevice(
          ...
          (res, err, _controls) => {
            ...
            if (res) {
              ...
              // Ignore transient decode errors to avoid UI spam
            }
          }
        );
        setControls(c);
  ...
  const toggleTorch = async () => {
    try {
      ...
      if (!track) return;
      // Some browsers support torch via applyConstraints
      // @ts-ignore
    ...
    <main style={{display:'grid', placeItems:'center', gap:16, padding:16, fontFamily:'system-ui'}}>
      ...
      {result && (
        <div style={{ maxWidth:600, width:'100%' }}>
          ...
          <p style={{fontSize:12, opacity:.8}}>Tip: supports EMVCo/NSPK links like <code>000201...</code> or <code>https://qr.nspk.ru/...</code></p>
        </div>
      )}
      {error && <p style={{ color:'crimson' }}>{error}</p>}
      <ul style={{ fontSize:13, opacity:.8 }}>
        <li>Hold camera 20–40cm away and avoid glare.</li>
        <li>Try the torch if the terminal’s screen is too dim or glossy.</li>
      </ul>
    </main>
  );
}
