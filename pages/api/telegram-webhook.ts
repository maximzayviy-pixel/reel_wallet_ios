...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // моментально отвечаем Telegram, чтобы не было «крутилки»
  ok(res);
  ...
  try {
    // 1) Подтверждаем pre_checkout_query — иначе у пользователя вечная «загрузка»
    ...
    // 2) Обрабатываем успешный платёж (Stars / XTR)
    const msg = update.message || update.edited_message;
    if (msg?.successful_payment && supabase) {
      const fromId = String(msg?.from?.id || '');
      const sp = msg.successful_payment;
      const currency = sp.currency;                 // ожидаем 'XTR'
      const total = Number(sp.total_amount || 0);   // у Stars = количество звёзд
      ...
      if (fromId && stars > 0) {
        ...
        // пробуем обновить/вставить баланс
        try {
          if (currentStars > 0) {
    ...
    // уже отдали 200 — ошибок наружу не даём, чтобы TG не ретраил
  }
}
